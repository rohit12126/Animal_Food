				...
				if( ( mfn_opts_get('subheader') != 'all' ) && 
					...
					if( $subheader_show ){
						if(is_post_type_archive("blog") || is_tax("blog_category")){
							$page_data = get_posts( array( 'name' => 'blog' , 'post_type' => 'page' ) );
							$sub_img = get_post_meta( $page_data[0]->ID, 'mfn-post-subheader-image', true );
						}else{
							$sub_img = get_post_meta( get_the_ID(), 'mfn-post-subheader-image', true );	
						}
						
						echo '<div class="breadcrumb-area pt-95 pb-95 bg-img" style="background-image:url('.$sub_img.');"><div class="container"><div class="breadcrumb-content text-center">';
						if( $title_show ){
							...
							echo '<h2>';
								...
								if(is_post_type_archive("blog")){
									$page_data = get_posts( array( 'name' => 'blog' , 'post_type' => 'page' ) );
									echo $page_data[0]->post_title;
								}elseif(is_tax("blog_category")){
									$page_data = get_posts( array( 'name' => 'blog' , 'post_type' => 'page' ) );
									echo $page_data[0]->post_title."-".get_query_var( 'term' );
								}else{
									the_title();
								}
