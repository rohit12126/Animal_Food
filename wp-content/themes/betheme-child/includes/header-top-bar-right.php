	...
	echo '<div class="top_bar_right">';
		echo '<div class="top_bar_right_wrapper">';
			...
			if( $header_search == 'input' ){
				...
				echo '<a id="search_button" href="#"><i class="icon-magnifier s-open"></i></a>';
				
			}

			// My Account
				echo '<a id="myaccount_button" href="'.get_site_url().'/my-account"><i class="icon-user icons"></i></a>';


			// WooCommerce Cart
			if( $has_cart ){
				echo '<a id="header_cart" href="'. $woocommerce->cart->get_cart_url() .'"><i class="icon-handbag"></i><span>'. $woocommerce->cart->cart_contents_count .'</span></a>';
			}
			// Languages menu
